...
impl Server {
    ...
    unsafe fn as_ptr(&self) -> *const UA_Server {
        unsafe { self.server.as_ptr() }
    }
    ...
    pub fn creates_event(
        ...
        let Some(event_id) = ua::EventId::new(out_event_id) else {
            return Err(Error::internal("createEvent should return event ID"));
        };
        Ok(event_id)
    }
    ...
    unsafe fn wait_for_config(
        ...
        let config = unsafe { &*UA_Server_getConfig(self.as_mut_ptr()) };
