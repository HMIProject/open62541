...
impl<K: MonitoredItemKind> MonitoredItemCreateRequestBuilder<K> {
    ...
    ///
    /// ```
    /// use open62541::{AsyncMonitoredItemBuilder, DataValue, MonitoredItemCreateRequestBuilder, MonitoredItemValue, ua};
...
///
/// This depends on the attribute ID passed to [`MonitoredItemCreateRequestBuilder::attribute_id()`].
...
impl MonitoredItemValue {
    ...
    /// This returns `None` for event monitored items.
    #[must_use]
    pub fn into_value(self) -> Option<ua::DataValue> {
        match self.0 {
            MonitoredItemValueInner::DataChange { value } => Some(value),
            MonitoredItemValueInner::Event { fields: _ } => None,
        }
    ...
    /// This returns `None` for data change monitored items.
    #[must_use]
    pub fn into_fields(self) -> Option<ua::Array<ua::Variant>> {
        match self.0 {
            MonitoredItemValueInner::DataChange { value: _ } => None,
            MonitoredItemValueInner::Event { fields } => Some(fields),
        }
    }
...
/// Sealed typestate used in [`MonitoredItemCreateRequestBuilder`].
...
/// Attribute for [`MonitoredItemCreateRequestBuilder::attribute()`].
