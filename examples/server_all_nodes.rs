...
use open62541_sys::{
    ...
    UA_NS0ID_HASCOMPONENT, UA_NS0ID_HASSUBTYPE, UA_NS0ID_OBJECTSFOLDER, UA_NS0ID_ORGANIZES,
    UA_NS0ID_STRING,
};
...
fn main() -> anyhow::Result<()> {
    ...
    let object_type_node_id = ua::NodeId::string(1, "PersonType");
    let object_type_node = Node {
        requested_new_node_id: object_type_node_id.clone(),
        parent_node_id: ua::NodeId::ns0(UA_NS0ID_BASEOBJECTTYPE),
        reference_type_id: ua::NodeId::ns0(UA_NS0ID_HASSUBTYPE),
        browse_name: ua::QualifiedName::new(1, "Person Type"),
        type_definition: None,
        attributes: ua::Attributes::ObjectType(ua::ObjectTypeAttributes::default()),
    };

    let person_variable_node_id = ua::NodeId::string(1, "name");
    let person_variable_node = Node {
        requested_new_node_id: person_variable_node_id.clone(),
        parent_node_id: object_type_node.requested_new_node_id.clone(),
    ...
    let person_object = Node {
        requested_new_node_id: ua::NodeId::string(1, "maintainer"),
        parent_node_id: object_node.requested_new_node_id.clone(),
        reference_type_id: ua::NodeId::ns0(UA_NS0ID_ORGANIZES),
        browse_name: ua::QualifiedName::new(1, "Maintainer"),
        type_definition: Some(object_type_node.requested_new_node_id.clone()),
        attributes: ua::Attributes::Object(ua::ObjectAttributes::default()),
    };

    let maintainer_variable = Node {
        requested_new_node_id: variable_node_id.clone(),
        parent_node_id: object_node.requested_new_node_id.clone(),
        reference_type_id: ua::NodeId::ns0(UA_NS0ID_ORGANIZES),
        browse_name: ua::QualifiedName::new(1, "the answer"),
    ...
    // https://www.open62541.org/doc/master/server.html#node-addition-and-deletion
    // https://www.open62541.org/doc/master/tutorial_server_object.html

    server.add_node(&object_node)?;
    server.add_node(&variable_node)?;

    server.add_node(&object_type_node)?;
    server.add_node(&person_variable_node)?;
    server.add_node(&person_object)?;
